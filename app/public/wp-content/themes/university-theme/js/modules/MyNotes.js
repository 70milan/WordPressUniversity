...
///((((here we need a delete request!!!!!))))
// for the delete button
    deleteNote(e) {
          var thisNote = $(e.target).parents("li");  
// url is what we set up earlier NONCE
            $.ajax({
            beforeSend: (xhr) => {
                xhr.setRequestHeader('X-WP-Nonce',uniData.nonce);
            },
            url: uniData.root_url + '/wp-json/wp/v2/notes/' + thisNote.data('id'),
            type: 'DELETE',
            success: (response)=> {
            // deleting "ON THE FLY"
                thisNote.slideUp();
            ...
            error: (response) => {
                console.log("ooopppppssss");
            console.log(response);
            }    
        });
    }
/// to make save button work
    updateNote(e) {
        var thisNote = $(e.target).parents("li");
        // url is what we set up earlier NONCE
        var ourUpdatedPost = {
...
//// making a brand new note outa scratch
    ...
    createNote(e) {
        ...
        $.ajax({
            ...
            url: uniData.root_url + '/wp-json/wp/v2/notes/',
            type: 'POST',
            data: ourNewPost,
            success: (response) => {
                // deleting "ON THE FLY"
